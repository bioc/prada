...
\begin{document}
\title{How to apply the ddCt method}
...
savedir <- function(x) file.path(.path.package("prada"), "extdata", x)
...
\Rfunction{eListWrite}.
<<write Tables>>=
eListWrite(result,file=savedir("allValues.txt"),sep="\t",row.names=FALSE)
write.table(assayData(result)$exprs,file=savedir("level_matrix.txt"), sep = "\t", col.names = NA)
write.table(assayData(result)$dCt,file=savedir("dCt_matrix.txt"), sep = "\t", col.names =NA)
@
