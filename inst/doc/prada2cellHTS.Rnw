...
\begin{document}
\title{Feeding the output of a cell-based assay into cellHTS}
...
using any of the available file handling functions provided by R or by
using any text processing software. 
...
files for each plate to plate and replicate idenitfiers. 
We first load the necessary R packages.
<<prepare>>=
library(prada)
library(cellHTS)
@ 
...
We ommit the normalization step since normalization has already been done
